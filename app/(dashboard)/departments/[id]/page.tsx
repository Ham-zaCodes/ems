...
interface Employee { _id: string; name: string; email: string; phone: string; position: string; status: string; joinDate: string }

function Avatar({ name }: { name: string }) {
  const initials = name.split(" ").map(n => n[0]).slice(0, 2).join("").toUpperCase();
  const gradients = ["from-indigo-500 to-violet-600","from-teal-400 to-cyan-600","from-pink-500 to-rose-500","from-amber-400 to-orange-500"];
...
export default function DepartmentDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      const [deptRes, empRes] = await Promise.all([fetch("/api/departments"), fetch(`/api/departments/${id}/employees`)]);
  ...
  if (loading) return (
    <div className="flex items-center justify-center py-24">
      <svg className="animate-spin w-8 h-8" style={{ color: "#8B5CF6" }} fill="none" viewBox="0 0 24 24">
        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
        <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
      </svg>
    </div>
  );
  if (!dept) return <p style={{ color: "rgba(255,255,255,0.5)" }}>Department not found.</p>;
    ...
    <div style={{ opacity: show ? 1 : 0, transform: show ? "translateY(0)" : "translateY(16px)", transition: "opacity 400ms ease, transform 400ms ease" }}>
      <div className="flex items-start justify-between mb-8">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center shadow-lg">
            <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
          </div>
          <div>
            <h2 className="text-2xl font-bold gradient-text">{dept.name}</h2>
            <p className="text-sm mt-0.5" style={{ color: "rgba(255,255,255,0.7)" }}>{dept.description || "No description"}</p>
          </div>
        </div>
        <Link href="/employees" className="btn-glass flex items-center gap-2 px-4 py-2.5 text-sm">
      ...
      <div className="grid grid-cols-3 gap-5 mb-8">
        {stats.map((s, i) => (
          <div key={s.label} className="glass-card p-5 flex items-center gap-4"
            style={{ opacity: show ? 1 : 0, transform: show ? "translateY(0)" : "translateY(16px)", transition: `opacity 400ms ease ${i * 80}ms, transform 400ms ease ${i * 80}ms` }}>
      ...
      <div className="glass-card overflow-hidden">
        <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
          ...
          <span className="text-xs px-2.5 py-1 rounded-full" style={{ background: "rgba(99,102,241,0.2)", color: "#a78bfa", border: "1px solid rgba(99,102,241,0.3)" }}>{employees.length} members</span>
        </div>
        <table className="glass-table">
          <thead><tr>{["Employee","Phone","Position","Status","Join Date"].map(h => <th key={h}>{h}</th>)}</tr></thead>
          <tbody>
            {employees.length === 0 ? (
              <tr><td colSpan={5} className="text-center py-16" style={{ color: "rgba(255,255,255,0.55)" }}>No employees in this department</td></tr>
            ) : employees.map((emp, i) => (
              <tr key={emp._id} style={{ opacity: show ? 1 : 0, transition: `opacity 300ms ease ${i * 40}ms` }}>
                ...
                <td style={{ color: "rgba(255,255,255,0.75)", fontSize: "0.75rem" }}>{emp.phone || "—"}</td>
                <td>{emp.position}</td>
                <td>
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold"
                    ...
                    <span className="w-1.5 h-1.5 rounded-full" style={{ background: emp.status === "active" ? "#14B8A6" : "#ef4444" }} />
