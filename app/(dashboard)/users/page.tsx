...
export default function UsersPage() {
  ...
  async function openEdit(u: User) {
    setEditing(u);
    // Try to load linked employee record
    const empRes = await fetch(
      `/api/employees?search=${encodeURIComponent(u.email)}&limit=1`,
    );
    const empData = await empRes.json();
    const linked = empData.employees?.[0];
    setForm({
      name: u.name,
      email: u.email,
      password: "",
      ...
      department:
        typeof u.department === "object"
          ? u.department?._id
          : u.department || linked?.department?._id || linked?.department || "",
      position: linked?.position || "",
      phone: linked?.phone || "",
      joinDate: linked?.joinDate ? linked.joinDate.slice(0, 10) : "",
    });
    setError("");
    setShowModal(true);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // If creating a new user, append it locally so it appears immediately in the list
    if (!editing) {
      setUsers((prev: any) => [data, ...prev]);
    } else {
      // If editing, re-fetch to get fresh list
      fetchUsers();
    }
    setShowModal(false);
  }
  ...
  async function handleDelete(id: string) {
    if (
      !confirm("Delete this user? Their employee record will also be removed.")
    )
      return;
    ...
    >
      ...
      <div className="glass-card overflow-hidden">
        <table className="glass-table">
          <thead>
            <tr>
              {["User", "Role", "Created", isAdmin ? "Actions" : ""]
                .filter(Boolean)
                .map((h) => (
                  <th key={h}>{h}</th>
                ))}
      ...
      {showModal && isAdmin && (
        <div
          className="fixed inset-0 flex items-center justify-center z-50 p-4 animate-overlay-in"
          style={{ background: "rgba(0,0,0,0.7)", backdropFilter: "blur(8px)" }}
        >
          <div
            className="glass-card w-full max-w-lg animate-modal-in"
            style={{
              ...
              <div>
                ...
                {editing ? (
                  ...
                  <div className="flex items-center gap-2">
                    <input readOnly value="Employee" className={inp} />
                    <input type="hidden" value="employee" />
                  </div>
                )}
              </div>

              {/* Employee details — shown automatically when role is employee */}
              {form.role === "employee" && (
                <div
                  className="space-y-4"
                  style={{
